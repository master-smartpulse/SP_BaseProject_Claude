...
- **Execução de tasks [P]**: despache lotes de tasks [P] da mesma fase como **subagents paralelos** (uma invocação por task, no mesmo turno), cada um com instruções autocontidas — o tasks.md garante que tasks [P] não compartilham arquivos. Os subagents **não fazem commits**: a consolidação, a validação da fase e o commit (por task, ou um por lote [P]) são seus, no contexto principal. Se o paralelismo não estiver disponível na sessão, execute sequencialmente na ordem dos IDs.
- **Mini-review por fase**: ao concluir cada fase de implementação (Fundação e cada User Story), despache o agente `review` como subagent sobre **apenas os arquivos alterados na fase**. O prompt do subagent deve conter: a lista de arquivos, os caminhos da spec/plan da feature e a instrução de **apenas reportar achados com severidade em formato resumido** — sem gravar arquivos (review.md é exclusivo do `/review` final). Achados **Críticos** bloqueiam o avanço: corrija e repasse o mini-review; após **2 tentativas** sem resolver, pare e reporte ao usuário com o contexto. Este mini-review não substitui o `/review` final do workflow.
- **Investigação de falhas**: em falha de teste ou bug sem causa clara durante a implementação, despache o agente `debugger` (read-only) para produzir evidência de causa raiz (arquivo:linha, saída) antes de tentar correções às cegas.
